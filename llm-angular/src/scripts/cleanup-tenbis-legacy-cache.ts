/**
 * Maintenance Script: Clean Legacy 10bis Cache Entries
 * 
...
/**
 * Clean legacy 10bis cache entries with fallback search URLs
 */
async function cleanupLegacy10bisSearchFallback(): Promise<void> {
  const startTime = Date.now();
  
  logger.info(
    { event: 'provider_cache_cleanup_started', provider: 'tenbis' },
    '[Cleanup] Starting 10bis legacy cache cleanup'
  );
  ...
  try {
    // Scan for all tenbis keys
    const pattern = 'provider:tenbis:*';
    const keys: string[] = [];
    ...
    logger.info(
      ...
      `[Cleanup] Found ${keys.length} tenbis cache keys`
    );
...
// Run cleanup
cleanupLegacy10bisSearchFallback()
  .then(() => {
