    ...
    try {
      ...
      await streamAssistantMessage(summaryContext, llmProvider, requestId, {
        onChunk: (text) => {
          ...
          // DEBUG: Log each delta in dev
          if (process.env.NODE_ENV !== 'production') {
            console.log('[SSE_DEBUG] delta', deltaCount, 'chars=', text.length, 'preview=', text.substring(0, 50));
          }
      ...
      // DEBUG: Log final message stats
      if (process.env.NODE_ENV !== 'production') {
        const lineCount = fullMessage.split('\n').length;
        console.log('[SSE_DEBUG] streamComplete=', {
          deltaCount,
          totalChars: fullMessage.length,
          lineCount,
          hasMultipleLines: lineCount > 1,
        });
        console.log('[SSE_DEBUG] fullMessage=', fullMessage);
      }

      this.logger.info(
        { requestId, traceId, language: assistantLanguage, deltaCount, messageLength: fullMessage.length, event: 'assistant_sse_summary_sent' },
