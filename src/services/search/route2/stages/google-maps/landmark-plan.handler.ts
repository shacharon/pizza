...
export async function executeLandmarkPlan(
  ...
  try {
    ...
    if (fromCache && beforeCount > resultsFiltered.length) {
      logger.debug(
        { requestId, event: 'google_places_cache_evicted_permanently_closed', beforeCount, afterCount: resultsFiltered.length, placeIds: results.filter((r: any) => r.businessStatus === 'CLOSED_PERMANENTLY').map((r: any) => r.placeId) },
