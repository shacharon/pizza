...
export function validateMessageFormat(
  ...
  if (messageSentences > MAX_MESSAGE_SENTENCES) errors.messageError = `Too many sentences (${messageSentences}, max ${MAX_MESSAGE_SENTENCES})`;
...
export function validateAndEnforceCorrectness(
  ...
  // 1) Language validation (script-based) - log only, do NOT replace LLM output (always LLM first)
  ...
  if (messageMismatch) {
    ...
    if (ASSISTANT_LANG_DEBUG) {
      ...
      }, '[ASSISTANT_DEBUG] Language mismatch detected - keeping LLM output');
    }
  } else if (ASSISTANT_LANG_DEBUG) {
    logger.info({
      requestId,
  ...
  // 2) Format validation - only format errors trigger deterministic fallback
