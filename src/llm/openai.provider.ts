...
export class OpenAiProvider implements LLMProvider {
    ...
    async completeJSON<T extends z.ZodTypeAny>(
        ...
        // Calculate prompt size (used in logging)
        const promptChars = messages.reduce((sum, m) => sum + (m.content?.length || 0), 0);

        // Helper to categorize errors for retry decision
        const categorizeError = (e: any) => {
            ...
            return { status, isAbortError, isTransportError, isParseError };
        };

        // Retry predicate: only retry transport errors (429, 5xx)
        const isRetryable = (e: any, attempt: number) => {
            const { isAbortError, isTransportError, isParseError } = categorizeError(e);
            ...
            // Parse errors: fail fast (shouldn't happen with Structured Outputs)
            if (isParseError) {
                return false;
            }
            
            // Only retry transport errors
            return isTransportError;
        };

        // Retry callback for logging
        const onRetry = (e: any, attempt: number, nextDelay: number) => {
            const status = e?.status ?? e?.code ?? e?.name;
            logger.warn({
                attempt: attempt + 1,
                maxAttempts,
                ...
                traceId: opts?.traceId
            }, '[LLM] Retriable transport error - will retry');
        };
        ...
        return await retryWithBackoff({
            ...
            fn: async () => {
                ...
                // Timing instrumentation: t2 = immediately before OpenAI call
                ...
                try {
                    ...
                    clearTimeout(t);
                    const { status, isAbortError, isTransportError, isParseError } = categorizeError(e);
                    ...
                    // Transport error - will be retried by retryWithBackoff
                    throw e;
                }
            }
        });
            ...
            const stream = await traceProviderCall(
                ...
                async () => {
                    ...
                    return await client.chat.completions.create({
                        ...
                        temperature,
                        stream: true,
                        // Ensure enough tokens for 4–6 sentence assistant summary (default can be low)
