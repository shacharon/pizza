...
export class SearchOrchestrator {
     ...
     */
    async search(request: SearchRequest, traceId?: string, requestId?: string, skipAssistant = false, forceV1 = false): Promise<SearchResponse> {
        ...
        if (SEARCH_PIPELINE_V2 && !forceV1) {
            ...
            const deps = createPipelineDependencies(
                gateAdapter,
                // Delegate function that calls V1 flow (force V1 to avoid recursion)
                (req, tid, rid, skip) => this.search(req, tid, rid, skip, true)
            );
