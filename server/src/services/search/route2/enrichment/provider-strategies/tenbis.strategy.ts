/**
 * Tenbis selection: top 5 results, name match required.
...
export function selectTenbisUrl(params: SelectUrlParams): string | null {
  ...
  for (const r of top5) {
    if (nameMatchesCandidate(name, r.title, r.url)) {
      logger.debug(
        ...
        '[TenbisStrategy] Name match in top 5, returning first match'
      );
      return r.url;
    }
  }
  ...
  logger.debug(
    {
      event: 'tenbis_strategy_no_name_match',
      requestedName: name,
      top5Urls: top5.map((r) => r.url),
    },
    '[TenbisStrategy] None of top 5 matched name, returning NOT_FOUND'
  );
  return null;
}
