...
export class PhraserService {
    private recentResponses = new Map<string, { response: string; timestamp: number; count: number }>();
    private lastTemplateIndex = new Map<string, number>();

    async phraseResults(opts: PhraseResultsOptions): Promise<string> {
        ...
        const recent = this.recentResponses.get(contextKey);
        const now = Date.now();
        ...
        let currentTemplateIndex = this.lastTemplateIndex.get(sessionId) ?? -1;
        currentTemplateIndex = (currentTemplateIndex + 1) % promptPool.length;
        this.lastTemplateIndex.set(sessionId, currentTemplateIndex);
        const selectedPrompt = promptPool[currentTemplateIndex] || promptPool[0];


        // Update tracking
        this.recentResponses.set(contextKey, {
...
export const phraserService = new PhraserService();
